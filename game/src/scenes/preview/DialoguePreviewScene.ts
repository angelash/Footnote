...
export class DialoguePreviewScene extends BasePreviewScene {
  ...
  protected createContent(width: number, _height: number): void {
    ...
    const stats = this.add
      .text(width / 2, currentY, `${sequences.length} 个对话序列`, {
    ...
    const description = this.add
      .text(width / 2, currentY, '点击对话可预览完整流程，支持选项分支', {
        fontFamily: 'Noto Sans SC',
  ...
  private createDialogueItem(
    ...
    const previewText = this.add.text(
      100,
      115,
      `"${firstLine.text.substring(0, 40)}${firstLine.text.length > 40 ? '...' : ''}"`,
      {
        fontFamily: 'Noto Sans SC',
        fontSize: this.FONT_SIZE.SMALL,
        color: '#4A4A4A',
  ...
  private playDialogue(sequence: IDialogueSequence): void {
    ...
    const titleText = this.add
      .text(width / 2, 40, `${sequence.chapter} - ${sequence.name}`, {
        fontFamily: 'Noto Sans SC',
    ...
    const progressText = this.add
      .text(width / 2, 85, `1 / ${sequence.lines.length}`, {
        fontFamily: 'Noto Sans SC',
        fontSize: this.FONT_SIZE.NORMAL,
        color: '#686868',
      })
