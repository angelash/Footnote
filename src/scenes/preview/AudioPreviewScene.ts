...
export class AudioPreviewScene extends BasePreviewScene {
  ...
  protected createContent(width: number, height: number): void {
    ...
    const stats = this.add
      .text(width / 2, currentY, `${categories.length} 个分类，共 ${totalAudio} 个音频`, {
  ...
  private createAudioItem(
    ...
    const volumeText = this.add
      .text(width - 250, height / 2, `音量: ${Math.round(item.volume * 100)}%`, {
        fontFamily: 'Noto Sans SC',
        fontSize: this.FONT_SIZE.SMALL,
        color: '#686868',
      })
    ...
    // 行交互
    container.setInteractive(
      new Phaser.Geom.Rectangle(0, 0, width - 100, height),
      Phaser.Geom.Rectangle.Contains
    );
