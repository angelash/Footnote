...
export class AudioPreviewScene extends BasePreviewScene {
  ...
  protected createContent(width: number, height: number): void {
    ...
    // 全局控制按钮
    const stopAllBtn = this.createStopAllButton(width / 2, currentY + 25);
    ...
    categories.forEach((category) => {
      ...
      category.items.forEach((item) => {
        const itemRow = this.createAudioItem(30, currentY, width - 60, 50, item, category.color);
  ...
  private createStopAllButton(x: number, y: number): Phaser.GameObjects.Container {
    ...
    bg.strokeRoundedRect(-60, -15, 120, 30, 6);
    container.add(bg);
  ...
  private createAudioItem(
    ...
    indicator.fillCircle(25, height / 2, 5);
    container.add(indicator);
    this.playingIndicators.set(item.id, indicator);

    // 音频名称
    const nameText = this.add.text(45, 10, item.name, {
      fontFamily: 'Noto Sans SC',
      fontSize: '14px',
      color: '#E8E6E3',
    });
    ...
    // 音频ID
    const idText = this.add.text(45, 30, item.id, {
      fontFamily: 'Noto Sans SC',
      fontSize: '10px',
      color: '#4A4A4A',
    });
    ...
    // 音量信息
    const volumeText = this.add.text(width - 180, height / 2, `音量: ${Math.round(item.volume * 100)}%`, {
      fontFamily: 'Noto Sans SC',
      fontSize: '11px',
      color: '#686868',
    }).setOrigin(0, 0.5);
    ...
    // 循环标记
    if (item.loop) {
      const loopText = this.add.text(width - 100, height / 2, '🔁', {
        fontSize: '14px',
      }).setOrigin(0, 0.5);
    ...
    // 播放按钮
    const playBtn = this.add.text(width - 50, height / 2, '▶', {
      fontSize: '20px',
      color: accentColor,
    }).setOrigin(0.5);
    ...
    // 行交互
    container.setInteractive(new Phaser.Geom.Rectangle(0, 0, width - 70, height), Phaser.Geom.Rectangle.Contains);
