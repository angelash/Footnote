...
export class DevPreviewScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // 标题
    this.titleText = this.add.text(width / 2, 60, '🛠️ 开发预览工具', {
    ...
    // 副标题
    this.add.text(width / 2, 100, '《备注 / Footnote》资源预览系统', {
      fontFamily: 'Noto Sans SC',
      fontSize: '14px',
      color: '#686868',
    }).setOrigin(0.5);
    ...
    // 版本信息
    this.versionText = this.add.text(width / 2, height - 30, 'DEV v0.1.0 | 按 ESC 返回此菜单', {
      fontFamily: 'Noto Sans SC',
      fontSize: '12px',
      color: '#4A4A4A',
    }).setOrigin(0.5);
  ...
  private createMenuItem(
    ...
    // 图标
    const icon = this.add.text(-width / 2 + 25, -height / 2 + 20, item.icon, {
      fontSize: '36px',
    });
    container.add(icon);

    // 标签
    const label = this.add.text(-width / 2 + 80, -height / 2 + 18, item.label, {
      fontFamily: 'Noto Sans SC',
      fontSize: '20px',
      color: '#E8E6E3',
      fontStyle: 'bold',
    ...
    // 描述
    const desc = this.add.text(-width / 2 + 80, -height / 2 + 48, item.description, {
      fontFamily: 'Noto Sans SC',
      fontSize: '13px',
      color: '#686868',
    });
