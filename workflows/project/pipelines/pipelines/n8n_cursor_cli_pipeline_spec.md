...
> 对齐最新落地：推荐直接使用 **Fixed Flow Pipeline**（`/webhook/fixed-flow`）完成“落盘审计 + 执行 + validate + git + notify”的端到端链路；旧版 `/webhook/execute-task` 仅作为兼容保留。
...
### 5.1 端口与实例
- **单入口实例（当前）**：`http://localhost:5680`
- **Windows 5678（可选）**：后续如需主从分发/浏览器任务再启用
